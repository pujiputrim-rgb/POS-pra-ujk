...
@section('page_subtitle', 'Masukkan data produk baru ke dalam inventaris.')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('products.index') }}" class="text-decoration-none text-muted-green">Products</a></li>
    <li class="breadcrumb-item active text-main" aria-current="page">Tambah Produk</li>
@endsection
            ...
            <div class="card-body p-5">
                <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-text text-muted-green mt-2"><i class="bi bi-info-circle me-1"></i> Format: JPG, PNG, GIF (Maks. 2MB)</div>
                        @error('photo')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control form-control-lg bg-light border-0 @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Kemeja Flanel, Sabun Cuci" required>
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="number" class="form-control form-control-lg bg-light border-0 @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price') }}" placeholder="150000" min="0" required>
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-5">
                            ...
                            <input type="number" class="form-control form-control-lg bg-light border-0 @error('stock') is-invalid @enderror" id="stock" name="stock" value="{{ old('stock', 0) }}" placeholder="10" min="0" required>
                            @error('stock')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
